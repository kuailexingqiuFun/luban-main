...
func (mc *MetricsCategory) GenerateQuery() *PrometheusQuery {
	...
	case "nodes":
		return &PrometheusQuery{
			...
			FsSize:                 fmt.Sprintf(`sum(node_filesystem_size_bytes{instance="%s"}) by (instance)`, mc.Nodes),
			FsUsage:                fmt.Sprintf(`sum(node_filesystem_size_bytes{instance="%s"} - node_filesystem_avail_bytes{instance="%s"}) by (instance)`, mc.Nodes, mc.Nodes),
			PodUsage:               fmt.Sprintf(`sum({__name__=~"kubelet_running_pod_count|kubelet_running_pods", node=~"%s"})`, mc.Nodes),
...
type MetricsQuery struct {
	...
	WorkloadMemoryUsage       *MetricsCategory `json:"workloadMemoryUsage"`
}
