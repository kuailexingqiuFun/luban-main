...
<script>
import { RolesList, RolesGet, RolesCreate, RolesUpdate, RolesDelete} from '@/api/kubernetes/roles'
...
export default {
  ...
  methods: {
    async getTableData(page = this.page, pageSize = this.pageSize, cluster_id = this.cluster_id, namespace = this.namespace, searchInfo = this.searchInfo.name) {
      const res = await RolesList(cluster_id, page, pageSize, namespace, searchInfo, '', '')
      if (res.code === 0) {
    ...
    // 添加
    async handleYAMLAdd(){
      this.currentValue =  getK8sObject("roles", this.namespace, "")
      this.title = "创建"
      this.dialogAddYamlVisible = true
    },
    async handleSubmitAdd(value){
      const res = await  RolesCreate(this.cluster_id, value.metadata.namespace, value)
      if (res.code) {
        this.$message({
          type: 'error',
    ...
    async handleEditYAML(value) {
      const res = await RolesGet(this.cluster_id, value.metadata.namespace, value.metadata.name)
      if (res.code === 0) {
    ...
    async handleSubmit(value) {
      this.dialogYamlVisible = false
      const res = await RolesUpdate(this.cluster_id, value.metadata.namespace, value.metadata.name, value)
      if (res.code !== 0) {
        this.$message({
          type: 'error',
    ...
    async handleDelete(value){
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        const res = await  RolesDelete(this.cluster_id, value.metadata.namespace, value.metadata.name, value)
        if (res.code === 0) {
          this.$message({
            type: 'success',
            message: '删除成功!'
