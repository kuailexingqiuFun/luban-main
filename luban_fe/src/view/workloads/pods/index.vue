...
<script>
import { PodsGet, PodsList, PodsUpdate, PodsCreate, PodsDelete} from '@/api/kubernetes/workloads/pods'
...
export default {
  ...
  methods: {
    async getTableData(page = this.page, pageSize = this.pageSize, cluster_id = this.cluster_id, namespace = this.namespace, searchInfo = this.searchInfo.name) {
      const res = await PodsList(cluster_id, page, pageSize, namespace, searchInfo, '', '')
      if (res.code === 0) {
    ...
    // 添加
    async handleYAMLAdd(){
      this.currentValue =  getK8sObject("pods", this.namespace, "")
      this.title = "创建"
      this.dialogAddYamlVisible = true
    },
    async handleSubmitAdd(value){
      const res = await  PodsCreate(this.cluster_id, value.metadata.namespace, value)
      if (res.code) {
        this.$message({
          type: 'error',
    ...
    async handleEditYAML(value) {
      const res = await PodsGet(this.cluster_id, value.metadata.namespace, value.metadata.name)
      if (res.code === 0) {
    ...
    async handleSubmit(value) {
      this.dialogYamlVisible = false
      const res = await PodsUpdate(this.cluster_id, value.metadata.namespace, value.metadata.name, value)
      if (res.code !== 0) {
        this.$message({
          type: 'error',
    ...
    async handleDelete(value){
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        const res = await  PodsDelete(this.cluster_id, value.metadata.namespace, value.metadata.name, value)
        if (res.code === 0) {
          this.$message({
            type: 'success',
            message: '删除成功!'
    ...
    handleTerminal(row, name){
      this.title = '容器终端 '+  ' 容器名称: ' + row.metadata.name
      this.dialogVisibleTerminal = true
      this.currentValuedetailurl = this.path + "/api/v1/kubernetes/proxy/terminal?pod_name=" + row.metadata.name + '&namespace=' + row.metadata.namespace + '&cluster_id=' + this.cluster_id + '&name='+ name
    },
    handleLogs(row, name){
      this.title = '容器日志 '+  ' 容器名称: ' + row.metadata.name
      this.dialogVisibleContainerlog = true
      this.currentValuedetailurl = this.path + "/api/v1/kubernetes/proxy/logs?pod_name=" + row.metadata.name + '&namespace=' + row.metadata.namespace + '&cluster_id=' + this.cluster_id + '&name='+ name
    },
    async podsDetail(value) {
      this.title = value.metadata.name
      const res = await PodsGet(this.cluster_id, value.metadata.namespace, value.metadata.name)
      if(res.code === 0){
        this.currentValue = res.data.items
        const resEvents = await EventsList(this.cluster_id, this.currentValue.metadata.name, this.currentValue.metadata.namespace)
        if (resEvents.code === 0) {
          this.currentValue["events"] = resEvents.data.items
        }
